...
#pragma HLS interface ap_fifo port=eout
  static half W_real[FF] = {0.0f};
  static half W_imag[FF] = {0.0f};
  static half ebuf[SHIFT] = {0.0f};
  half X_real[FF], X_imag[FF];
  half Ax[NPblk];
  half Px[NPblk];
  half D_real[NPblk], D_imag[NPblk];
  half Y_real[FF], Y_imag[FF];    
  half dtmp[FF];
  half etmp[S];
  float Ay[NPblk];
  half E_real[FF], E_imag[FF];
  float Ae[NPblk];
  half tmp[FF];
  float Px_ave[NPblk];
  float Ratio_ex[NPblk];
  float Ratio_yx[NPblk];
  half Etmp_real[NPblk], Etmp_imag[NPblk];
  half EE_real[NPblk], EE_imag[NPblk], EEE[NPblk];
  half dW_real[FF], dW_imag[FF];
  half dD_real[FF], dD_imag[FF];
  half e[SHIFT];
  half sum_E, sum_dD, gamma2;
  int i, j, k, ii;
  static half xbuff[FF] = {0.0f};
#pragma HLS BIND_STORAGE variable=xbuff type=ram_2p impl=bram  
  static half ytmp[FF] = {0.0f};
...
#include "wwin.h"  
  ...
  fft<half,half,half,half,float>(Ax, 1, Px, 1, X_real, X_imag, xbuff, 0, FF, rp_x, WE_win, 0);
  ...
  // Dを逆FFTして，時間信号へ
  ifft<half, half>(dtmp, D_real, D_imag);

  // 1ブロック分のエコキャン処理実行
  for (i = 0; i < S; i++) {
    etmp[i] = ytmp[(i + rp_y) % FF] - dtmp[L + i];
  }
  fft<float,half,half,half,float>(Ay, 1, Px, 0, Y_real, Y_imag, ytmp, L, S, rp_y, WE_win, 1);
  ...
  for (j = 0; j < LOOP; ++j) {
    ...
    // （残差信号のスペクトル）
    fft<float,half,half,half,float>(Ae, 1, Px, 0, E_real, E_imag, etmp, L, S, 0, WE_win, 1);      
    ...
    ifft<half,half>(tmp, W_real, W_imag);

    // 時間領域で拘束をかけた更新ベクトルを再度周波数領域へ
    fft<half,half,half,half,float>(Ax, 0, Px, 0, W_real, W_imag, tmp, 0, L, 0, WE_win, 1);
    ...
    // 更新ベクトルを時間領域へ
    ifft<half,half>(tmp, dW_real, dW_imag);
    
    // 時間領域で拘束をかけた更新ベクトルを再度周波数領域へ        
    fft<half,half,half,half,float>(Ax, 0, Px, 0, dW_real, dW_imag, tmp, 0, L, 0, WE_win, 1);
    
    // 更新したフィルタで残差に残ったエコー成分を再度予測
    mul(dD_real, dD_imag, X_real, X_imag, dW_real, dW_imag);
    
    // 時間領域に戻して
    ifft<half, half>(dtmp, dD_real, dD_imag);
    
    // 残差に残ったエコー成分の予測値のスペクトル
    fft<half,half,half,half,float>(Ax, 0, Px, 0, dD_real, dD_imag, dtmp, L, S, L, WE_win, 1);
  ...
  ifft<half, float>(E_real, cmpE_real, cmpE_imag);
